...
const PATH_SOCIAL_ACCOUNT = path.resolve(
  __dirname,
  "../../onchain/target/dev/afk_ignite_MintStablecoin.contract_class.json"
);
const PATH_SOCIAL_ACCOUNT_COMPILED = path.resolve(
  __dirname,
  "../../onchain/target/dev/afk_ignite_MintStablecoin.compiled_contract_class.json"
);

/** @TODO spec need to be discuss. This function serve as an example */
export const createMintStablecoin = async (
  PRAGMA_ADDRESS: string,
  PRAGMA_STATISTIC_ADDRESS: string,
  name: string,
  symbol: string,
  recipient: string,
  decimals: number,
  ...
  try {
    ...
    const constructorCalldata = CallData.compile([{
      PRAGMA_ADDRESS: PRAGMA_ADDRESS,
      PRAGMA_STATISTIC_ADDRESS: PRAGMA_STATISTIC_ADDRESS,
      recipient: recipient,
      token_address: token_address,
      decimals: decimals,
      name: byteArray.byteArrayFromString(name),
      symbol: byteArray.byteArrayFromString(symbol),
      token_id: cairo.felt(token_id),
    }])
